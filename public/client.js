...
const API_URL = "https://project-babel.ddns.net";
...
async function sendCommand(action) {
    ...
    try {
        ...
        // 1. 顯示網路延遲
        if (netTime > 0) {
            const ram = data.metrics && data.metrics.memory ? ` | MEM: ${Math.round(data.metrics.memory/1024)}KB` : '';
            appendLog(`[SYSTEM] NET: ${netTime}ms${ram}`, 'cost-msg');
        }
